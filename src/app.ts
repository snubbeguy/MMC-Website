import type { ExportResult } from "./generator.shared";
...
const seedInput = document.querySelector("#seedInput") as HTMLInputElement;
const radiusInput = document.querySelector("#radiusInput") as HTMLInputElement;
const formatSelect = document.querySelector("#formatSelect") as HTMLSelectElement;
const resolutionSelect = document.querySelector("#resolutionSelect") as HTMLSelectElement;
const previewButton = document.querySelector("#previewButton") as HTMLButtonElement;
const downloadColoredButton = document.querySelector("#downloadColoredButton") as HTMLButtonElement;
const downloadUncoloredButton = document.querySelector("#downloadUncoloredButton") as HTMLButtonElement;
const retryDownloadButton = document.querySelector("#retryDownloadButton") as HTMLButtonElement;
const downloadDialog = document.querySelector("#downloadDialog") as HTMLDialogElement;
const downloadDialogTitle = document.querySelector("#downloadDialogTitle") as HTMLHeadingElement;
const closeDownloadDialogButton = document.querySelector("#closeDownloadDialogButton") as HTMLButtonElement;
const cancelDownloadDialogButton = document.querySelector("#cancelDownloadDialogButton") as HTMLButtonElement;
const confirmDownloadButton = document.querySelector("#confirmDownloadButton") as HTMLButtonElement;
const rawCanvas = document.querySelector("#rawCanvas") as HTMLCanvasElement;
const previewCanvas = document.querySelector("#previewCanvas") as HTMLCanvasElement;
const rawContext = rawCanvas.getContext("2d")!;
const previewContext = previewCanvas.getContext("2d")!;
const statusText = document.querySelector("#statusText") as HTMLParagraphElement;
const previewSize = document.querySelector("#previewSize") as HTMLElement;
const downloadSize = document.querySelector("#downloadSize") as HTMLElement;
const blockSpan = document.querySelector("#blockSpan") as HTMLElement;
...
interface Camera {
  ...
  drag: { canvas: HTMLCanvasElement; pointerId: number; x: number; y: number } | null;
}
...
let generatorModulePromise: Promise<typeof import("./generator.shared")> | null = null;
let busyMode: string | null = null;
let lastDownload: { blob: Blob; filename: string } | null = null;
let pendingDownloadSource = "uncolored";
let backendMode: string | null = location.protocol === "file:" ? "main" : null;
...
async function getGeneratorModule(): Promise<typeof import("./generator.shared")> {
  if (!generatorModulePromise) {
    generatorModulePromise = import("./generator.shared");
  }
...
function openDownloadDialog(source: string = pendingDownloadSource): void {
  if (busyMode) return;
...
function buildBasePayload(): { seed: string; radius: number } | null {
...
function applyPreviewResult(message: any): void {
  busyMode = null;
  setBusy(false);
...
function applyExportResult(message: ExportResult & { buffer?: ArrayBuffer }): void {
  busyMode = null;
  setBusy(false);
...
function rememberDownload(blob: Blob, filename: string): void {
...
function clearLastDownload(): void {
  lastDownload = null;
...
function createRgbSourceCanvas(pixels: Uint8Array, width: number, height: number): HTMLCanvasElement {
...
function drawEmptyPane(context: CanvasRenderingContext2D, width: number, height: number): void {
...
function getView(canvasWidth: number, canvasHeight: number): { x: number; y: number; width: number; height: number } {
...
function clampCamera(canvasWidth: number = previewCanvas.width, canvasHeight: number = previewCanvas.height): void {
...
function screenToSource(canvas: HTMLCanvasElement, offsetX: number, offsetY: number): { x: number; y: number } {
  const rect = canvas.getBoundingClientRect();
  const view = getView(canvas.width, canvas.height);
  return {
