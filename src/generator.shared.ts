...
import {
  ...
  WORLDGEN_NOISE,
} from "./data/vanilla-overworld-data";
...
interface Engine {
  ...
  temperatureSampler: { compute(ctx: ClimateContext): number };
  vegetationSampler: { compute(ctx: ClimateContext): number };
  continentsSampler: { compute(ctx: ClimateContext): number };
  erosionSampler: { compute(ctx: ClimateContext): number };
  ridgesSampler: { compute(ctx: ClimateContext): number };
  surfaceSampler: { compute(ctx: ClimateContext): number };
}
...
interface GenerateMaskResult {
  width: number;
  height: number;
  blocksPerPixel: number;
  durationMs: number;
  pixels: Uint8Array;
  rawPixels: Uint8Array | null;
}

interface RenderDownloadOptions {
  seed: string;
  radius: number;
  blocksPerPixel: number;
  format: string;
...
const SEA_LEVEL = (OVERWORLD_NOISE_SETTINGS as { sea_level: number }).sea_level;

let worldgenRegistered = false;
let sharedSettings: NoiseGeneratorSettings | null = null;
let cachedEngine: Engine | null = null;

export async function generateMask(options: GenerateMaskOptions): Promise<GenerateMaskResult> {
...
export async function renderDownload(options: RenderDownloadOptions): Promise<ExportResult> {
  const startedAt = performance.now();
  const engine = getEngine(options.seed);
...
function ensureWorldgenRegistered(): void {
  if (worldgenRegistered) return;

  for (const [id, json] of Object.entries(WORLDGEN_NOISE)) {
    WorldgenRegistries.NOISE.register(Identifier.parse(id), NoiseParameters.fromJson(json as any), true);
  }

  for (const [id, json] of Object.entries(WORLDGEN_DENSITY_FUNCTIONS)) {
    WorldgenRegistries.DENSITY_FUNCTION.register(
      Identifier.parse(id),
      DensityFunction.fromJson(json as any),
      true,
    );
  }

  sharedSettings = NoiseGeneratorSettings.create({
    noise: NoiseSettings.fromJson((OVERWORLD_NOISE_SETTINGS as any).noise),
    surfaceRule: SurfaceRule.NOOP,
    defaultBlock: BlockState.fromJson((OVERWORLD_NOISE_SETTINGS as any).default_block),
    defaultFluid: BlockState.fromJson((OVERWORLD_NOISE_SETTINGS as any).default_fluid),
    noiseRouter: NoiseRouter.fromJson((OVERWORLD_NOISE_SETTINGS as any).noise_router),
    seaLevel: (OVERWORLD_NOISE_SETTINGS as any).sea_level,
    disableMobGeneration: (OVERWORLD_NOISE_SETTINGS as any).disable_mob_generation,
    aquifersEnabled: (OVERWORLD_NOISE_SETTINGS as any).aquifers_enabled,
    oreVeinsEnabled: (OVERWORLD_NOISE_SETTINGS as any).ore_veins_enabled,
    legacyRandomSource: (OVERWORLD_NOISE_SETTINGS as any).legacy_random_source,
  });

  worldgenRegistered = true;
}

function getEngine(seedText: string): Engine {
  ...
  const randomState = new RandomState(sharedSettings!, seed);
  const climateContext: ClimateContext = { x: 0, y: 0, z: 0 };
  const climateSample: ClimateSample = {
    temperature: 0,
    vegetation: 0,
    continents: 0,
    erosion: 0,
  ...
  cachedEngine = {
    ...
    climateContext,
    climateSample,
    temperatureSampler: randomState.router.temperature as any,
    vegetationSampler: randomState.router.vegetation as any,
    continentsSampler: randomState.router.continents as any,
    erosionSampler: randomState.router.erosion as any,
    ridgesSampler: randomState.router.ridges as any,
    surfaceSampler: randomState.router.preliminarySurfaceLevel as any,
  };

  return cachedEngine;
}

function normalizeSeed(seedText: string): bigint {
...
function writePreviewPoint(engine: Engine, blockX: number, blockZ: number, pixels: Uint8Array, rawPixels: Uint8Array | null, offset: number): void {
...
function sampleClimate(engine: Engine, blockX: number, blockZ: number): ClimateSample {
...
function biomeForClimate(climate: ClimateSample): string {
...
function isWaterPoint(biome: string, climate: ClimateSample): boolean {
...
function waterColorFor(biome: string): number[] {
...
function buildFilename(source: string, seedText: string, radius: number, blocksPerPixel: number): string {
...
function buildSampleCoordinates(width: number, radius: number, blocksPerPixel: number): Int32Array {
...
function createRenderPlan(options: { radius: number; blocksPerPixel: number; source: string }): RenderPlan {
  const diameter = options.radius * 2 + 1;
  return {
...
function writeColorAt(engine: Engine, blockX: number, blockZ: number, source: string, output: Uint8Array | Uint8ClampedArray, offset: number): void {
  const climate = sampleClimate(engine, blockX, blockZ);
  const biome = biomeForClimate(climate);
...
function writeMaskColor(biome: string, climate: ClimateSample, output: Uint8Array | Uint8ClampedArray, offset: number): void {
...
function writeBiomeColor(biome: string, climate: ClimateSample, output: Uint8Array | Uint8ClampedArray, offset: number): void {
  if (isWaterPoint(biome, climate)) {
...
async function encodeGeneratedPng(engine: Engine, plan: RenderPlan, onProgress: (msg: string) => void): Promise<Uint8Array> {
  ...
  const compressedPromise = new Response((stream.readable as ReadableStream<Uint8Array>).pipeThrough(new CompressionStream("deflate") as any)).arrayBuffer();
...
async function encodeGeneratedJpeg(engine: Engine, plan: RenderPlan, onProgress: (msg: string) => void): Promise<Blob> {
...
function assemblePng(width: number, height: number, compressed: Uint8Array): Uint8Array {
  const chunks = [
    pngSignature(),
