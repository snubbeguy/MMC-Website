...
(function (SpecialRenderers) {
    ...
    const shieldFace = (index) => ({
        south: { uv: [0.5, 0.5, 3, 5.5], texture: `#${index}`, tintindex: index },
    });
    function shieldRenderer(atlas, layers) {
        const patternFn = (index) => ({
            from: [-5, -10, -0.99],
            to: [5, 10, -0.99],
            faces: shieldFace(index),
        });
        const textures = { 0: 'entity/shield/shield_base_nopattern' };
        const elements = [
            {
                from: [-6, -11, -2],
                to: [6, 11, -1],
        ...
        return new BlockModel(undefined, textures, elements)
            .getMesh(atlas, Cull.none(), (index) => DyeColors[tints[index] ?? ''] ?? [1, 1, 1]);
    }
    ...
    function createBannerRenderer(color, config) {
        return (atlas, patterns) => {
            const textures = { 0: 'entity/banner/banner_base' };
    ...
    function copperGolemStatueRenderer(texture) {
        return (atlas, pose) => {
    ...
    SpecialRenderers.copperGolemStatueRenderer = copperGolemStatueRenderer;
